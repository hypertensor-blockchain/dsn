...
from petals.validator.from_pretrained import FromPretrainedMixin as FromPretrainedMixinValidator
from petals.validator.lm_head import LMHead as LMHeadValidator
from petals.validator.ptune import PTuneMixin as PTuneMixinValidator
from petals.validator.remote_generation import RemoteGenerationMixin as RemoteGenerationMixinValidator
from petals.validator.remote_generation import RemotePastKeyValues as RemotePastKeyValuesValidator
from petals.validator.remote_sequential import RemoteSequential as RemoteSequentialValidator
from petals.models.llama.config import DistributedLlamaConfigValidator
...
class DistributedLlamaModelValidator(FromPretrainedMixinValidator, PTuneMixinValidator, LlamaModel):
    ...
    def __init__(self, config: DistributedLlamaConfigValidator, *, dht: Optional[hivemind.DHT] = None):
        n_layer, config.num_hidden_layers = config.num_hidden_layers, 0  # Prevent initialization
        super().__init__(config)
        assert len(self.layers) == 0
        config.num_hidden_layers = n_layer

        self.layers = RemoteSequentialValidator(config, dht=dht)
...
class DistributedLlamaForCausalLMValidator(FromPretrainedMixinValidator, RemoteGenerationMixinValidator, LlamaForCausalLM):
    ...
    def __init__(self, config: DistributedLlamaConfigValidator):
        LlamaPreTrainedModel.__init__(self, config)
