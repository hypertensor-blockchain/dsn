...
from subnet.client.from_pretrained import FromPretrainedMixin
from subnet.client.lm_head import LMHead
from subnet.client.ptune import PTuneMixin
from subnet.client.remote_generation import RemoteGenerationMixin, RemotePastKeyValues
...
from subnet.validator.from_pretrained import FromPretrainedMixin as FromPretrainedMixinValidator
from subnet.validator.lm_head import LMHead as LMHeadValidator
from subnet.validator.ptune import PTuneMixin as PTuneMixinValidator
from subnet.validator.remote_generation import RemoteGenerationMixin as RemoteGenerationMixinValidator
from subnet.validator.remote_generation import RemotePastKeyValues as RemotePastKeyValuesValidator
from subnet.validator.remote_sequential import RemoteSequential as RemoteSequentialValidator
from subnet.models.llama.config import DistributedLlamaConfigValidator
...
class DistributedLlamaModelValidator(FromPretrainedMixinValidator, PTuneMixinValidator, LlamaModel):
    """LlamaModel, but all transformer layers are hosted by the swarm"""

    _keys_to_ignore_on_load_missing = PTuneMixinValidator._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = [r"^model\.layers\."]
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[RemotePastKeyValuesValidator] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        if past_key_values is None:
            past_key_values = RemotePastKeyValuesValidator()
        past_key_values.update_seen(hidden_states.size(1))
    ...
    @property
    def h(self) -> RemoteSequentialValidator:  # For compatibility with RemoteGenerationMixin
        return self.layers
...
class DistributedLlamaForCausalLMValidator(FromPretrainedMixinValidator, RemoteGenerationMixinValidator, LlamaForCausalLM):
    _keys_to_ignore_on_load_missing = DistributedLlamaModelValidator._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedLlamaModelValidator._keys_to_ignore_on_load_unexpected
    ...
    @property
    def transformer(self) -> DistributedLlamaModelValidator:  # For compatibility with RemoteGenerationMixin
        return self.model


class DistributedLlamaForSequenceClassificationValidator(FromPretrainedMixin, LlamaForSequenceClassification):
    _keys_to_ignore_on_load_missing = DistributedLlamaModelValidator._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedLlamaModelValidator._keys_to_ignore_on_load_unexpected
    ...
    def __init__(self, config):
        ...
        self.model = DistributedLlamaModelValidator(config)
        self.score = nn.Linear(config.hidden_size, self.num_labels, bias=False)
