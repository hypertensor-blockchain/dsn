...
from subnet.utils.cuda_graphs import make_inference_graphed_callable
...
class OptimizedLlamaAttentionValidator(LlamaAttention):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._rotary_graph = None
...
class OptimizedLlamaDecoderLayerValidator(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size
        self.self_attn = OptimizedLlamaAttentionValidator(config=config, layer_idx=0)
...
class WrappedLlamaBlockValidator(OptimizedLlamaDecoderLayerValidator):
    def forward(
        self,
        hidden_states: torch.Tensor,
        *args,
