...
from subnet.validator.routing.sequence_info import RemoteSequenceInfo
from subnet.validator.routing.spending_policy import NoSpendingPolicy
from subnet.data_structures import ModuleUID, RemoteSpanInfo, ServerState
from subnet.server.handler import TransformerConnectionHandler
from subnet.utils.dht import get_remote_module_infos
...
class SequenceManagerConfig(ClientConfig):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "subnet.client.routing.SequenceManagerConfig has been moved to subnet.ClientConfig. "
...
class RemoteSequenceManager:
        ...
        *,
        ...
        if config.initial_peers or dht is None:
            try:
                ...
                config.initial_peers = f_initial_peers_tuple
    ...
    def _make_sequence_with_specific_peers(
        ...
        """
        :param peers: Peers data {peer_id, start, end} in ascending order from start:end range, i.e. 0:2, 3:14, 15:32, etc.
        ...
        while current_index < end_index:
            ...
            if current_index in (i['start'] for i in peers):
                ...
                # i.e. peer has 20:30, update chosen_span to 14:19
                for span in candidate_spans:
                    for peer in peers:
                        if chosen_span.start <= peer['start'] <= chosen_span.end:
    ...
    def _make_sequence_with_specific_peers_single_block(
        self, 
        peers: List[Dict], 
        start_index: int, 
        end_index: int
        ...
        # copy peers list to check they are in the sequence later
        peers_to_verify = peers
        ...
        for i in range(len(peers_to_verify)):
            found = False

            for j in range(span_sequence):
                if span_sequence[i].peer_id == peers_to_verify[j].peer_id:
                    found = True
                    break

            if not found:
                logger.error(f"Peer not found in make_sequence: %s" % peers_to_verify[j].peer_id)
                return False
...
class MissingBlocksError(RuntimeError):
    def __init__(self, block_indices: Union[int, Sequence[int]]):
        super().__init__(
            ...
            f"You can check the public swarm's state at https://health.subnet.dev "
            f"If there are not enough servers, please connect your GPU: "
            f"https://github.com/bigscience-workshop/subnet#connect-your-gpu-and-increase-subnet-capacity "
        )
