...
async def ping(
    peer_id: hypermind.PeerID,
    _dht: hypermind.DHT,
    node: hypermind.dht.DHTNode,
    *,
...
async def ping_parallel(peer_ids: Sequence[hypermind.PeerID], *args, **kwargs) -> Dict[hypermind.PeerID, float]:
...
class PingAggregator:
    def __init__(self, dht: hypermind.DHT, *, ema_alpha: float = 0.2, expiration: float = 300):
        self.dht = dht
        self.ema_alpha = ema_alpha
        self.expiration = expiration
        self.ping_emas = hypermind.TimedStorage()
        self.lock = threading.Lock()

    def ping(self, peer_ids: Sequence[hypermind.PeerID], **kwargs) -> None:
        current_rtts = self.dht.run_coroutine(partial(ping_parallel, peer_ids, **kwargs))
        logger.debug(f"Current RTTs: {current_rtts}")

        with self.lock:
            expiration = hypermind.get_dht_time() + self.expiration
    ...
    def to_dict(self) -> Dict[hypermind.PeerID, float]:
